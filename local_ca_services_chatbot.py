"""
CA Services Chatbot with Local Embeddings

A Streamlit app for the CA Services RAG Chatbot that uses local embeddings from the OUTPUT folder
instead of connecting to Pinecone.
"""
...
try:
    ...
    # Load local embeddings from OUTPUT folder
    try:
        st.info("Loading local embeddings from OUTPUT folder...")

        # List of embedding files to load
        embedding_files = [
            "OUTPUT/list_documents_ca_services_embeddings.json",
            "OUTPUT/comprehensive_regulatory_services_embeddings.json"
        ]
        ...
        for file_path in embedding_files:
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    st.success(f"Successfully loaded {len(embeddings)} embeddings from {file_path}")
            except Exception as e:
                st.warning(f"Failed to load embeddings from {file_path}: {str(e)}")

        if not all_embeddings:
            st.error("No embeddings were loaded. Please check the OUTPUT folder.")
            st.stop()

        st.success(f"Successfully loaded {len(all_embeddings)} total embeddings")

    except Exception as e:
        st.error(f"Failed to load local embeddings: {str(e)}")
    ...
    def search_local_embeddings(query, top_k=5):
        """Search local embeddings with a query string."""
        # Get query embedding
...
Remember, your goal is to help users understand accounting, tax, and financial advisory services accurately and provide valuable information regardless of what's in the context."""
    ...
    if prompt := st.chat_input("Ask a question about CA Services..."):
        ...
        with st.chat_message("assistant", avatar="🤖"):
            ...
            try:
                # Get relevant context from local embeddings
                results = search_local_embeddings(prompt, top_k=TOP_K_RESULTS)
    ...
    st.markdown("""
    ---
    *Powered by OpenAI GPT-4o and Local Embeddings*
    """)
